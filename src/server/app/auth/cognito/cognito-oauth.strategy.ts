...
export class CognitoOauthStrategy extends PassportStrategy(
	...
	async validate(accessToken: string) {
		...
		const [provider, providerId] = userinfo.username.includes('_') ? userinfo.username.split('_') : ['unknown', 'unknown'];

		let user = await this.usersService.findOneByProviderAndUsername(providerId, userinfo.username);
		if (!user) {
